...
async def register_user(
    ...
    try:
        ...
        await registration_service.register_user(user_data=user_data)
        request.session["email"] = user_data.email

        return RedirectResponse(url="/api/v1/auth/verify-email", status_code=302)
...
async def post_verify_email(
    ...
    try:
        await verification_service.verify_email(email=email, code=code)
        request.session.pop("email", None)

        return RedirectResponse("/api/v1/auth/login", status_code=302)

    except HTTPException as e:
        return templates.TemplateResponse(
            "verify_email.html", {"request": request, "email": email, "error": e.detail}
        )
